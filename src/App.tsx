...
export default function App() {
  ...
  useEffect(() => {
    if (auth && auth.currentUser) {
      auth.currentUser.getIdTokenResult().then(res => {
        setIsAdminUser(Boolean(res.claims.admin));
  ...
  const handleStatusToggle = async (selectedEv: TourEvent) => {
    ...
    try {
      ...
      showToast(`⚡ [${cityName}] 상태가 '${statusName}'(으)로 Firestore에 실시간 동기화되었습니다!`);
    } catch (err: any) {
      console.error('[StatusToggle] Failed:', err);
      showToast(`⚠️ 상태 변경 실패: 관리자 권한(admin)이 필요합니다.`);
    }
  };
  ...
  const handleApproveNews = async (newsId: string) => {
    try {
      ...
      showToast('⚠️ 뉴스 승인 실패: 관리자 권한이 필요합니다.');
    }
  };
  ...
  const handleRejectNews = async (newsId: string, reason: string) => {
    try {
      ...
      showToast('⚠️ 뉴스 반려 실패: 관리자 권한이 필요합니다.');
    }
  };
  ...
  const handleApproveLang = async (contentId: string) => {
    try {
      ...
      showToast('⚠️ 학습 표현 승인 실패: 관리자 권한이 필요합니다.');
    }
  };
  ...
  const handleRejectLang = async (contentId: string) => {
    try {
      ...
      showToast('⚠️ 학습 표현 반려 실패: 관리자 권한이 필요합니다.');
    }
  };
