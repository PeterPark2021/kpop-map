...
async function seed() {
  console.log('🚀 Firestore languageContent 컬렉션에 실제 데이터 주입 중...');
  for (const item of languageData) {
    await setDoc(doc(db, 'languageContent', item.contentId), item);
    console.log(`✓ Firestore 저장 완료: [${item.koreanText}] (${item.reviewStatus})`);
  }
  console.log('🎉 Firestore languageContent 실제 주입 100% 완료!');
  process.exit(0);
}
